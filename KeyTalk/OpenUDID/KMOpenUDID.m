...
//    }
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                ...
                (unsigned long long)(arc4random() % 4294967295)];
    }
...
//
+ (NSString*) value {
    return [KMOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [KMOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [KMOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [KMOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
